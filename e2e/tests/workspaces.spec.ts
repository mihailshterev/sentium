...
test.describe.serial("Workspaces", () => {
  ...
  test("creates a new workspace", async ({ workspacesPage }) => {
  ...
  test("selects a workspace and shows file upload area", async ({ workspacesPage, page }) => {
    await workspacesPage.selectWorkspace(WORKSPACE_NAME);
    await expect(page.getByRole("button", { name: /upload/i })).toBeVisible();
  });

  test("deletes a workspace", async ({ workspacesPage, page }) => {
